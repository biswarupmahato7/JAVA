...
public class selection_sort {
    static void selectionSort(int arr[]) {
        ...
        for (int i = 0; i < n - 1; i++) { // i represent the Current Index
            // fiend the min index in unsorted array
            int min_index = i;
            for (int j = i + 1; j < n; j++) { // traverse the rest of the array and if u find any element less then
            ...
            if (min_index != i) { // if both are equal == no need to swap
                int temp = arr[i];
                arr[i] = arr[min_index];
                arr[min_index] = temp;
